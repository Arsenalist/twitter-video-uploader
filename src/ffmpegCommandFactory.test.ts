...
describe('FfmpegCommandFactory', function () {
    ...
    describe('createStripAudioCommand', function () {
        it("executes ffmpeg command with -an option preceding out file", () => {
            expect(factory.createStripAudioCommand("infile", "outfile")).toEqual(`${ffmpegBinary} -y -i infile -c copy -an outfile`)
        })
    })

    describe('creatTrimCommand', function () {
        it("sets in and out correctly when both are provided", () => {
            expect(factory.creatTrimCommand(5, 7, "infile", "outfile")).toEqual(`${ffmpegBinary} -y -i infile -ss 5 -to 7 outfile`)
        })
        it("sets in only when out isn't provided", () => {
            expect(factory.creatTrimCommand(5, undefined, "infile", "outfile")).toEqual(`${ffmpegBinary} -y -i infile -ss 5 outfile`)
        })
        it("sets out only when in isn't provided", () => {
            expect(factory.creatTrimCommand(undefined, 7, "infile", "outfile")).toEqual(`${ffmpegBinary} -y -i infile -to 7 outfile`)
        })
